	...
	u8 rpwm_val, bool b_need_turn_off_ckk)
{
	...
	if (IS_IN_LOW_POWER_STATE_8821AE(rtlhal->fw_ps_state)) {
		...
		if (FW_PS_IS_ACK(rpwm_val)) {
			...
			RT_TRACE(COMP_POWER, DBG_LOUD, ("Receive CPWM INT!!! Set rtlhal->FwPSState = %X\n", rtlhal->fw_ps_state));
			}
		}
...
void rtl8821ae_set_hw_reg(struct ieee80211_hw *hw, u8 variable, u8 *val)
{
	...
	case HW_VAR_BASIC_RATE:{
			...
			//rtlpriv->cfg->ops->set_hw_reg(hw, HW_VAR_INIT_RTS_RATE, val);
			break;
		}
	case HW_VAR_BSSID:{
			for (idx = 0; idx < ETH_ALEN; idx++) {
...
bool _rtl8821ae_check_pcie_dma_hang(struct ieee80211_hw *hw)
{
	...
	if ((tmp & BIT(0)) || (tmp & BIT(1))) {
		RT_TRACE(COMP_INIT, DBG_LOUD, ("CheckPcieDMAHang8821AE(): true! Reset PCIE DMA!\n"));
		return true;
	} else {
		return false;
	}
...
int rtl8821ae_hw_init(struct ieee80211_hw *hw)
{
	...
	if (b_support_remote_wakeup && rtlhal->b_wake_from_pnp_sleep &&
	...
	u8 *hwinfo)
{
	...
	RT_TRACE(COMP_INIT, DBG_LOUD, ("hal_ReadPowerValueFromPROM8821ae(): hwinfo[0x%x]=0x%x\n", (eeAddr+1), hwinfo[eeAddr+1]));
